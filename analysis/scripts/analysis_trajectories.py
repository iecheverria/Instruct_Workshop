...
class AnalysisTrajectories(object):
    ...
    def do_extract_models(self, gsms_info, filename, gsms_dir, path=None):
        ...
        # Setup a list of processes that we want to run
        processes = [mp.Process(target=self.extract_models, args=(df_array[x], filename, gsms_dir, path)) for x in range(self.nproc)]
    ...
    def extract_models(self, gsms_info, filename, gsms_dir, path = None):
        '''
        ...
        for row in gsms_info.itertuples():
            ...
            file = row.rmf3_file
            if path:
                traj_in = os.path.join(path, id, file)
            else:
